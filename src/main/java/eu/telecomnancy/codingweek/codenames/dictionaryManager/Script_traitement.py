...
def reorganiser_ordreFrequenceMot(liste_mots_selec):
    ...
    for k in range(4, 0, -1):
        for i in range(len(liste_mots_selec)):
            if (i+k)%4 == 0:
                liste_mots_selec_reorganise(liste_mots_selec[i])
...
def traiter_champLexical(codenamesWords_path):
    ...
    liste_intersection = intersection(liste_mots_selec_reorganise, liste_mots_dico)
    return liste_intersection
...
def obtenir_nouvelIdMot(mot, codenamesWords_path):
    ...
    nouvel_id_mot = liste_mots_dico.index(mot)
    return nouvel_id_mot


def ajouter_champLexical_fichier(mot, codenamesWords_path, ChampLexical_path):
    ...
    fichier = open(ChampLexical_path, 'w')
    fichier.write("".join(nouvelle_liste_champLexical))
    fichier.close()
